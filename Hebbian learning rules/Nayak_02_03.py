...
class HebbianLearning:
    ...
    def __init__(self, root, master, *args, **kwargs):
        ...
        self.decay_slider = tk.Scale(self.sliders_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
                                             from_=0.1, to_=1.0, resolution=0.01, bg="#DDDDDD", length=self.length_widget,
                                             activebackground="#FF0000",
                                             highlightcolor="#00FFFF",
                                             label="Decay rate",
                                             command=lambda event: self.decay_rate_slider_callback())
        self.decay_slider.set(self.decay)
        self.decay_slider.bind("<ButtonRelease-1>", lambda event: self.decay_rate_slider_callback())
        self.decay_slider.grid(row=1, column=0)  # , sticky=tk.N + tk.E + tk.S + tk.W

        self.weight_slider = tk.Scale(self.sliders_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
                                     from_=0.001, to_=1.0, resolution=0.001, bg="#DDDDDD", length=self.length_widget,
                                     activebackground="#FF0000",
                                     highlightcolor="#00FFFF",
                                     label="Weight from [-wieght to weight]",
                                     command=lambda event: self.weight_slider_callback())
        self.weight_slider.set(self.weight)
        self.weight_slider.bind("<ButtonRelease-1>", lambda event: self.weight_slider_callback())
        self.weight_slider.grid(row=2, column=0)  # , sticky=tk.N + tk.E + tk.S + tk.W

        self.epoch_slider = tk.Scale(self.sliders_frame, variable=tk.IntVar(), orient=tk.HORIZONTAL,
                                      from_=100, to_=1000, resolution=50, bg="#DDDDDD", length=self.length_widget,
                                      activebackground="#FF0000",
                                      highlightcolor="#00FFFF",
                                      label="Epochs",
                                      command=lambda event: self.epoch_slider_callback())
        self.epoch_slider.set(self.epochs_per_click)
        self.epoch_slider.bind("<ButtonRelease-1>", lambda event: self.epoch_slider_callback())
        self.epoch_slider.grid(row=3, column=0)  # , sticky=tk.N + tk.E + tk.S + tk.W
        #########################################################################
        ...
        self.root.status_bar.set("%s","I Have more Slider and default as mentioned per slider functionality, but you can change those per experiment.")


    def learning_rate_slider_callback(self):
        self.learning_rate = self.learning_rate_slider.get()

    def decay_rate_slider_callback(self):
        self.decay = self.decay_slider.get()

    def epoch_slider_callback(self):
        self.epochs_per_click = self.epoch_slider.get()

    def weight_slider_callback(self):
        self.weight = self.weight_slider.get()

        if self.weight != self.weight_max:
            self.root.top_frame.set('%s', "You have changed weights, Do you want to regenerate random weights before training ?")

        self.weight_max = self.weight
        self.weight_min = -1 * (self.weight)
    ...
    def test(self,epoch):
        ...
        errorRate = (lentr - correct_classification) / lentr
